...
Custom iOS notification sounds must be bundled as native iOS resources at the app bundle root. In practice, use `.wav`, `.aiff`, or `.caf` files and ensure they are copied into your app target during the native build.

If you are using Expo, importing a sound through `expo-assets` or `expo-notifications` does not automatically make it available to `UNNotificationSound`. If you currently store notification sounds as `.mp3`, convert them to a supported notification-sound format first and then copy them into the native iOS target during `expo prebuild` or native project setup. If you use the official `@psync/notifee` Expo config plugin, you can automate this with the `iosSoundFiles` option. The plugin copies the files into the app target and the generated Notification Service Extension target during the same prebuild pass, placing them at the bundle root so they can be referenced by filename only, for example `sound: 'chime.wav'`.

If you use a Notification Service Extension to modify remote notifications, make sure any custom sound file referenced by the notification is available to the extension flow as well.

...
If your remote notifications use a custom iOS sound, prefer `iosSoundFiles` so the sound is copied into both the app target and the generated Notification Service Extension during prebuild.

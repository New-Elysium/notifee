...
If you only need standard APNs features such as `sound`, `category`, or other `aps` fields, prefer the plain APNs payload path first. Reserve `notifee_options` for cases where you genuinely need Notifee to mutate the incoming notification content or attach rich media through a Notification Service Extension.
...
If you are using Expo, `@psync/notifee` now ships an official config plugin that can automate this setup during `expo prebuild`, including the extension target, Podfile wiring, entitlements, and EAS app-extension metadata.

For Expo projects, set the main iOS deployment target with `expo-build-properties`, and use the Notifee plugin for the extension-specific work.
...
If your remote notifications use a custom iOS sound, prefer `iosSoundFiles` so the sound is copied into both the app target and the generated Notification Service Extension during prebuild. The plugin places the file at the app bundle root, so reference it by filename only (for example `sound: 'chime.wav'`).

If you need to customize the generated extension further, the plugin also supports `serviceExtensionSettings.name`, `bundleIdentifier`, `deploymentTarget`, `appGroupName`, `customSourceFilePath`, `entitlements`, and `infoPlist`.

If you prefer to manage the native iOS project manually, the steps below still work.
...
target 'NotifeeNotificationService' do
  pod 'RNNotifeeCore', :path => '../node_modules/@psync/notifee/RNNotifeeCore.podspec'
end
...
> If your remote notification references a custom iOS sound, that sound must already exist in the native bundle used by the app/extension. Runtime JS asset imports are not enough on their own.
...
## Interception Control with `@react-native-firebase/messaging`

By default, Notifee intercepts iOS remote notification tap events. If you use `@react-native-firebase/messaging` and need its `onNotificationOpenedApp` / `getInitialNotification` methods to work, configure Notifee to opt out of interception using the `setNotificationConfig` API:

```js
import notifee from '@psync/notifee';

// Allow Firebase Messaging to handle remote notification taps on iOS
...
See the [FCM integration guide](/react-native/docs/integrations/fcm) for more details on FCM setup and configuration options.

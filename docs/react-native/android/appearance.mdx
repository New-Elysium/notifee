...
Since badges are only available on Android 8.0 and above, the devices will also have [channel](/react-native/android/channels)
...
```js
import notifee, { AndroidBadgeIconType } from '@psync/notifee';
...
```js
import notifee, { AndroidImportance } from '@psync/notifee';
...
```js
import notifee, { AndroidVisibility } from '@psync/notifee';

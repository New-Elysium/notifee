...
If a notification is pressed with no action, a `PRESS` [event](/react-native/events) published to any subscribers.
...
[App Open Events](/react-native/events#app-open-events) documentation to learn how to read the notification
...
them away. This can be prevented by creating an ongoing notification (see [Behaviour](/react-native/behaviour#ongoing))
or with [Foreground Service](/react-native/foreground-service) notifications.
...
View the [events](/react-native/events) documentation for more about handling events.
...
<Vimeo id="385481038" caption="Android Actions Example" />
...
At a minimum, each `pressAction` requires a unique ID, which can be used with [events](/react-native/events) to
...
View the [App Open Events](/react-native/events#app-open-events) documentation to learn more about finding the

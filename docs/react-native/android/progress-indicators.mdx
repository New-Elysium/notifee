...
- `points[].position` must use the same unit as `segments[].length`.
...
If `shortCriticalText` is an empty string, Notifee still forwards it to Android. This lets OEMs
or the platform intentionally fall back to other compact metadata when rendering promoted ongoing
chips.

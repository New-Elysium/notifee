...
- **Remote notifications** — first-class FCM and OneSignal interop (`handleFcmMessage`, `setFcmConfig`, plus a build-only `buildFcmMessage` flow for inspecting or patching payloads), plus a Notification Service Extension helper for iOS rich/modified payloads. ([Integrations](#integrations-fcm-onesignal))
...
- **Firebase Cloud Messaging (FCM)** — use `notifee.handleFcmMessage(remoteMessage)` to convert an FCM data/notification message into a Notifee display, or `buildFcmMessage(remoteMessage)` to build the `Notification` object without displaying it (inspect, patch, then `displayNotification()` yourself). Configure defaults via `setFcmConfig({ ... })`. See the [FCM integration guide](https://notifee.app/react-native/integrations/fcm).

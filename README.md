...
- **Scheduling & triggers** — display notifications at a time, on an interval, or at a location using `createTriggerNotification`. Defaults to Android `AlarmManager` for reliable delivery when the app is killed; opt out with `alarmManager: false`. Supports `repeatInterval` multipliers (every 2 days, every 3 months) and `RepeatFrequency.MONTHLY` calendar recurrence. ([Triggers](https://notifee.app/react-native/docs/triggers))
- **Foreground services (Android)** — long-running tasks with a prominent, ongoing notification and action controls. Defaults to `FOREGROUND_SERVICE_IMMEDIATE` so the notification shows without the system-imposed 10-second delay on Android 12+. ([Guide](https://notifee.app/react-native/docs/android/foreground-service))
...
- **Remote notifications** — first-class FCM and OneSignal interop (`handleFcmMessage`, `setFcmConfig`, plus a build-only `buildFcmNotification` flow for inspecting or patching payloads), plus a Notification Service Extension helper for iOS rich/modified payloads. ([Integrations](#integrations-fcm-onesignal))
...
**iOS sound files** are placed at the app bundle root so they can be referenced by filename only (for example `sound: 'chime.wav'`). The plugin resolves the host app's Xcode target via the Expo mod system's `modRequest.projectName`, with a fallback to `name` and finally to the project's first target — so `iosSoundFiles` work even when the `name` in your app config does not match the Xcode target. If the target can still not be resolved, prebuild emits a warning and the sound files are copied to `ios/NotifeeSounds/` but **not** added to the bundle — in that case re-run `expo prebuild --clean` and confirm the app config `name` (or the Expo-idiomatic project name) matches the Xcode target.
...
- **Firebase Cloud Messaging (FCM)** — use `notifee.handleFcmMessage(remoteMessage)` to convert an FCM data/notification message into a Notifee display, or `buildFcmNotification(remoteMessage)` to build the `Notification` object without displaying it (inspect, patch, then `displayNotification()` yourself). Configure defaults via `setFcmConfig({ ... })`. See the [FCM integration guide](https://notifee.app/react-native/integrations/fcm).

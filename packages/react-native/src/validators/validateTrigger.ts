...
// or a Date.getDate() day-of-month). We intercept the common shapes and emit
// targeted hints (see upstream invertase/notifee#872).
...
function validateTimestampTrigger(trigger: TimestampTrigger): TimestampTrigger {
  ...
  if (objectHasProperty(trigger, 'alarmManager') && !isUndefined(trigger.alarmManager)) {
    ...
    // Default to AlarmManager for reliable delivery when the app is killed
    out.alarmManager = validateTimestampAlarmManager();
  }
  ...
  // MONTHLY and custom repeatInterval multipliers are currently only implemented
  // on Android (native AlarmManager recurrence). The iOS rolling schedule for
  // these is not yet ported — reject with a clear error instead of silently
  // creating a non-repeating trigger.
  if (isIOS) {
    if (out.repeatFrequency === RepeatFrequency.MONTHLY) {
      throw new Error("'trigger.repeatFrequency' MONTHLY is not yet supported on iOS.");
    }
    if (out.repeatInterval !== undefined && out.repeatInterval > 1) {
      throw new Error("'trigger.repeatInterval' greater than 1 is not yet supported on iOS.");
    }
  }
